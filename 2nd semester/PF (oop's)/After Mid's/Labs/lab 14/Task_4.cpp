...
class mexception33: public exception
{
    private:
        ...
        int code;
    public:
        mexception33(string msg, int c): message(msg), code(c){}
...
class Rational
{
    ...
    public:
        ...
        Rational(float n, float d)
        {
            ...
            if(d==0)
            {
                throw mexception33("Denominator can't be 0", 102);
                return;
            }
            den=d;
        }
    ...
    for(int i = 0; i < 3; i++)
        ...
        void setdata(int i, int j, T value)
        {
            if(i>=0 && i<3 && j>=0 && j<3)
                ...
                throw mexception33("Index out of range in matrix33", 101);
                return ;}
        }
...
identityComplexMatrix.display();
    ...
    cout << "Matrix Exception: " << e.what() << " | Code: " << e.getCode() << '\n';
}
