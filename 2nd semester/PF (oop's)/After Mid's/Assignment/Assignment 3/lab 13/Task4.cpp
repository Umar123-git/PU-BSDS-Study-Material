...
class Sensible : public Player {
    Board* board; // Weak aggregation with Board
public:
    Sensible(char mark, Board* b) : Player(mark), board(b) {}

    int getMove() {
        char b[9];
        board->getBoard(b); // Get the current state of the board
        char opponentMark = (getMark() == 'O') ? 'X' : 'O';

        // Rule 1: Check if there are 2 marks in any row, column, or diagonal and the 3rd is free
        for (int i = 0; i < 3; i++) {
            // Check rows
