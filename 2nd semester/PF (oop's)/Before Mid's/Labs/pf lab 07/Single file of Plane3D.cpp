...
class Plane2D
{
	...
	public:
		...
		Plane2D(float _a , float _b , float _c , float _d)
		{
			a=_a;
			b=_b;
			c=_c;
		...
		bool twoplanesequal(const Plane2D& p)
		{
			...
			return (a/p.a==b/p.b) && (b/p.b==c/p.c) && (c/p.c==d/p.d);
		...
		{
			cout<<"\nDistance : ";
			double absolute= abs(a*point.getx() + b*point.gety() + c*point.getz() + d);
			double sqroot  = sqrt((a*a) + (b*b) + (c*c));
