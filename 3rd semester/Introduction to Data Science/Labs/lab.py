...
for entry in structured_logs:
    ...
    if doctor not in data:
        data[doctor]=[]
    data[doctor].append(hr)


avg={}
for doctor, hr in data.items():
    avg[doctor] = sum(hr)/len(hr)

for doctor, avg in avg.items():
    print(f"doctor {doctor}: Average Speed = {avg:.2f}")
